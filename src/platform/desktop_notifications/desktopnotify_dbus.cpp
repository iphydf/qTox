...
#ifdef QTOX_NOTIFY_IMAGE_DATA
// https://specifications.freedesktop.org/notification-spec/1.2/hints.html
// image-data (iiibiiay)
QVariant imageDataFromPixmap(const QPixmap& pixmap)
{
    ...
    return QVariantList{
        ...
        4,
        // data (ay)
        QVariant::fromValue(std::vector<unsigned char>(image.bits(), image.bits() + image.sizeInBytes())),
    };
}
#endif  // QTOX_NOTIFY_IMAGE_DATA
} // namespace
...
#ifdef QTOX_NOTIFY_IMAGE_DATA
            {d->imageDataFieldName(), imageDataFromPixmap(pixmap)},
#endif
        };
