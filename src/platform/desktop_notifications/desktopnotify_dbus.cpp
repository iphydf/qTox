...
struct DesktopNotifyDBus::Private
{
...
bool DesktopNotifyDBus::showMessage(const QString& title, const QString& message, const QPixmap& pixmap)
{
    // Try Notify first.
    if (d->notifyInterface.isValid()) {
        const QFile* tempFile = nullptr;
        if (QTOX_NOTIFY_TEMP_FILE) {
            tempFile = getIconFile(pixmap.toImage(), d->iconMutex, d->iconFile);
            if (tempFile == nullptr) {
                qWarning() << "Failed to create temporary file for notification image";
                return false;
            }
        }
        const QVariantMap hints{
            ...
            tempFile == nullptr
                ? std::make_pair(imageDataFieldName(d->specVersion),
                                 QVariant::fromValue(DBusNotifyImage{pixmap.toImage()}))
                : std::make_pair(QStringLiteral("image-path"), tempFile->fileName()),
        };
